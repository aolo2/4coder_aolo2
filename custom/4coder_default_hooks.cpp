...
default_render_buffer(Application_Links *app, View_ID view_id, Face_ID face_id,
    ...
    if (token_array.tokens != 0){
        ...
        ARGB_Color color_function = fcolor_resolve(fcolor_id(defcolor_index_function));
        ARGB_Color color_macro = fcolor_resolve(fcolor_id(defcolor_index_macro));
        ARGB_Color color_type = fcolor_resolve(fcolor_id(defcolor_index_type));
        ...
        for (;;){
            ...
            if (note != 0) {
                ...
                } else if (note->note_kind == CodeIndexNote_Function || note->note_kind == CodeIndexNote_FunctionPrototype) {
...
#if 1
    {
        ...
        if(token && (token->kind == TokenBaseKind_Identifier 
                     ...
                     || token->kind == TokenBaseKind_LiteralFloat))
        {
            Scratch_Block scratch(app);
            String_Const_u8 token_text = push_token_lexeme(app, scratch, buffer, token);
            
            Range_i64 visible_range = text_layout_get_visible_range(app, text_layout_id);
            i64 first_index = token_index_from_pos(&token_array, visible_range.first);
            it = token_iterator_index(0, &token_array, first_index);
            
            Rect_f32 range_start_rect = text_layout_character_on_screen(app, text_layout_id, token->pos);
            Rect_f32 range_end_rect = text_layout_character_on_screen(app, text_layout_id, token->pos + token->size - 1);
            ...
            ARGB_Color highlight_color = fcolor_resolve(fcolor_id(defcolor_pop3));
            
            draw_rectangle(app, total_range_rect, 0.0f, highlight_color);
            
            if (token && token->kind == TokenBaseKind_Identifier) {
                for (;;) {
                    Token *other_token = token_it_read(&it);
                    if (token->pos >= visible_range.one_past_last){
                        break;
                    }
                    
                    if (other_token && token != other_token) {
                        String_Const_u8 other_token_text = push_token_lexeme(app, scratch, buffer, other_token);
                        
                        if (string_match(token_text, other_token_text)) {
                            range_start_rect = text_layout_character_on_screen(app, text_layout_id, other_token->pos);
                            range_end_rect = text_layout_character_on_screen(app, text_layout_id, other_token->pos + other_token->size - 1);
                            total_range_rect.x0 = Min(range_start_rect.x0, range_end_rect.x0);
                            total_range_rect.y0 = Min(range_start_rect.y0, range_end_rect.y0);
                            total_range_rect.x1 = Max(range_start_rect.x1, range_end_rect.x1);
                            total_range_rect.y1 = Max(range_start_rect.y1, range_end_rect.y1);
