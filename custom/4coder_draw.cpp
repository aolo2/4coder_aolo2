...
draw_file_bar(Application_Links *app, View_ID view_id, Buffer_ID buffer, Face_ID face_id, Rect_f32 bar){
    ...
    FColor bar_bg = fcolor_id(defcolor_bar);
    
    Dirty_State dirty = buffer_get_dirty_state(app, buffer);
    
    if (HasFlag(dirty, DirtyState_UnsavedChanges)){
        bar_bg = fcolor_id(defcolor_file_unsaved);
    }
    
    if (HasFlag(dirty, DirtyState_UnloadedChanges)){
        bar_bg = fcolor_id(defcolor_file_unloaded);
    }
    
    draw_rectangle_fcolor(app, bar, 0.f, bar_bg);
    ...
    push_fancy_stringf(scratch, &list, base_color, " %lld:%lld", cursor.line, cursor.col);
    ...
    switch (*eol_setting){
        ...
        {
            push_fancy_string(scratch, &list, base_color, string_u8_litexpr(" [UNIX]"));
        }break;
        
        case LineEndingKind_CRLF:
        {
            push_fancy_string(scratch, &list, base_color, string_u8_litexpr(" [WINDOWS]"));
        }break;
    }
