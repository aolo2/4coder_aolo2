        ...
        if (file != 0){
            for (i32 i = 0; i < file->note_array.count; i += 1){
                ...
                switch (note->note_kind){
                    ...
                    case CodeIndexNote_FunctionPrototype:
                    {
...
CUSTOM_UI_COMMAND_SIG(jump_to_definition_file)
CUSTOM_DOC("List all definitions in the file and jump to one chosen by the user.")
{
    char *query = "Definition:";
    ...
    if (file != 0){
        for (i32 i = 0; i < file->note_array.count; i += 1){
            ...
            lister_add_item(lister, note->verbose_text, sort, jump, 0);
        }
    }
    code_index_unlock();
